...
#'
#' @return $results: statistical object, p-values adjusted
#'
#' @examples
#' \dontrun{# correct the statistical comparison "comp" using effective dimension method
#' ... %>%
#'  mt_post_effdim(stat_name="comp") %>% ...
#'  }
#'
#' @author KC
#'
...
mt_post_effdim <- function(D,
                        stat_name,
                        p_col = p.value  # needed for forcing early evaluation of column selection
                        ){
  # validate argument
  ...
  metadata(D)$results[[stat_id]]$output$table %<>%
    dplyr::mutate(p_adj=(!!p_col * effdim) %>%
                    pmin(.,1))
...
#'
#' @param X dataframe with samples as rows and metabolites as columns
#'
