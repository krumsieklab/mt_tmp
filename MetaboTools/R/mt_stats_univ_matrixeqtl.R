...
#'
#' @export
mt_stats_univ_lm_matrixeqtl <- function(D, formula, stat_name, sample_filter) {
  ...
  Y$CreateFromMatrix(assay(D[,samples.used])) # leave in metabolites X samples form
  ...
  # we want to model metabolite ~ outcome + covariates
