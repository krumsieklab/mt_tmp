...
#' @examples
#' \dontrun{...  %>%
#'   mt_logging_statsinfo(stat_name="comp1", stat_filter=p.adj<0.2) %>% ...}
#'
#' @author JK
#'
#' @importFrom magrittr %>% %<>%
...
mt_logging_statsinfo <- function(D, stat_name, stat_filter) {
  ...
  allstats <- D %>% MetaboTools::mtm_res_get_path(c("stats"))
  ...
  if (!missing(stat_filter)) {
    ...
    # add to log text
    logtxt <- sprintf("%s /// metabolites: %s, %d of %d", logtxt, s, hits, output$table %>% nrow())
  }
