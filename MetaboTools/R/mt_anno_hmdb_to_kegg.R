...
#'
#' Creates annotations and merges them into current SummarizedExperiment. Performs "left-joins", i.e. leaves the original SE
#' unchanged and just adds information where it can be mapped.
#'
...
mt_anno_hmdb_to_kegg <- function(D, in_col, out_col = "KEGG") {
  ...
  load(system.file("extdata", "precalc/pathview/MetaboliteMapping.Rds", package = "MetaboTools"))
  # get rowData
  ...
  dd <- utils::read.csv2(file=system.file("extdata", "precalc/pathview/MetaboliteMapping_manual.csv", package = "MetaboTools"), sep=",")
