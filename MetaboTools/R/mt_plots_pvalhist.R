...
mt_plots_pvalhist <- function(
  ...
  if (is.null(statnames)) statnames <- D %>% mti_res_get_stats_entries() %>% purrr::map("output") %>% purrr::map("name") %>% unlist()
  ...
  metadata(D)$results %<>%
    mti_generate_result(
      funargs = funargs,
      logtxt = glue::glue("P-value histograms for {paste0(statnames, collapse=', ')}"),
      output = plots
    )
