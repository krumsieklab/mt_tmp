...
mt_stats_univ_lm <- function(
  ...
  if (stat_name %in% unlist(MetaboTools::mtm_res_get_stats_entries(D) %>% purrr::map("output") %>% purrr::map("stat_name"))) stop(sprintf("stat element with name '%s' already exists",stat_name))
