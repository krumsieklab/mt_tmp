...
#'
#' @examples
#' \dontrun{%>% mt_modify_aggPW(pw_col="SUB_PATHWAY", method="aggmean") %>%  # subpathways from metabolon
#'
...
#'   mt_modify_aggPW(pw_col="kegg_db", method="aggmean") %>%}
#'
#' @author JK
#'
#' @importFrom magrittr %>% %<>%
...
mt_modify_aggPW <- function(
  ...
  # get variable
  if (!(pw_col %in% colnames(rowData(D)))) stop(sprintf("'%s' not found in metabolite annotations.", pw_col))
  p = rowData(D)[[pw_col]]
