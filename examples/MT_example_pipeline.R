...
D <-
  # validate checksum
  mt_other_checksum(file=file_data, checksum = "80afcd72481c6cf3dcf83342e3513699") %>%
  ...
  mt_anno_xls(file=file_data, sheet="metinfo",anno_type="metabolites", anno_id="name", data_id="name") %>%
  # load clinical (colData) annotations
  mt_anno_xls(file=file_data, sheet="clin", anno_type="samples", anno_id="sample", data_id="sample") %>%
...
#   - mt_other_settings - set global settings for MetaboTools pipeline
#   - mt_other_flag_logged - for flagging a loaded dataset as already log transformed
...
D <- D %>%
  ...
  mt_modify_filter_metabolites(filter = !is.na(SUB_PATHWAY)) %>%
...
D <- D %>%
  ...
  mt_plots_missingness(met_max=0.5) %>%
  # filter metabolites with more than 50% missing values per group
  mt_pre_filter_missingness(met_max = 0.5, group_col = "Diagnosis") %>%
...
D1 <- D1 %>%
  ...
  mt_write_stats(file = "AgeAnalysis.xlsx", met_col =  "BOCHEMICAL") %>%
  # pvalue histogram
