...
imputeKNN <- function(dat,
  ...
  if("knn.vars" %in% methods) {
    ...
    if(verbose)message(paste0("Number of imcomplete variables: ", length(incom.vars)))
      ...
      # also manuell:
      D2 <- as.matrix(dist(t(scale(dat)),upper=T,diag=T)) # eucl
      D2[D2==0] <- NA
      ...
      for (i in incom.obs){   
        ...
        D2 <- sapply(1:nrow(dat),function(j) tryCatch(mahalanobis(obs_i[!is.na(datpart[j,])],na.omit(as.numeric(datpart[j,])),Sx_i[!is.na(datpart[j,]),!is.na(datpart[j,])]),error=function(e) NA ))
        D2[D2==0] <- NA
        if(any(!is.na(D2))) {
      ...
      if(verbose)message(paste0("Number of imcomplete observations: ", length(incom.obs)))
      if(verbose)message("Proceeding obs... ")
      D2 <- as.matrix(dist(scale(dat),upper=T,diag=T)) # eucl
      D2[D2==0] <- NA
      for (i in incom.obs){ 
        if(verbose)cat(paste(i," "))                                       
        comvars <-  complete.cases(as.numeric(dat[i,]))
