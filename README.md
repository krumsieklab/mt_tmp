...
**Note:** Maplet is in active development. Any commit without a release tag is not guaranteed to be stable. Omission of the release tag (@v1.0.0) when installing Maplet via devtools will install the most recent commit from the master branch.

## Getting Started
Users should review the available examples in the [mt/examples](https://gitlab.com/krumsieklab/mt/-/tree/master/examples) folder. A large example pipeline demonstrating the general setup of a Maplet pipeline and most of the funcitons provided by Maplet is provided [here](https://gitlab.com/krumsieklab/mt/-/blob/master/examples/MT_example_pipeline.R). The mt/example folder also contains several stand-alone examples for more specialized functions not included in the main example.  
