...
MTResultCollector <- R6::R6Class("MTResultCollector", list(
  ...
  add = function(D) {
    ...
    for (i in 1:length(r)) {
      ...
      na <- self$graph %>% vertex_attr("name")
      if (!(r[[i]]$uuid %in% na)) {
        self$graph <- self$graph + vertex(r[[i]]$uuid, label=paste0(r[[i]]$fun,collapse="_"))
        print(sprintf('adding node %s',r[[i]]$uuid))
      }
      # add edge
      if (i>1) {
        self$graph <- self$graph + edge(r[[i-1]]$uuid,r[[i]]$uuid)
  ...
  graph_roots = function() {
    i <- which(sapply(sapply(V(self$graph), function(x) neighbors(self$graph,x, mode="in")), length) == 0)
    (self$graph %>% vertex_attr("name"))[i]
  },
  
  # get next node for a given vertex ID
  graph_next = function(id) {
    stopifnot(length(id)==1)
    ne <- neighbors(self$graph, id, 'out')
